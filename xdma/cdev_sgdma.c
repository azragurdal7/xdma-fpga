...
/* ------------------------------------------------------------------ */
/* Double Buffer (ping-pong) — C2H LFSR sürekliliği                   */
/* ------------------------------------------------------------------ */

/*
 * Bir slot'un DMA'sı bittiğinde interrupt/poll thread bu fonksiyonu çağırır.
 * Slot'u "filled" olarak işaretler ve okuyan tarafı (char_sgdma_read) uyandırır.
 * Hata durumunda dbuf_active=false yaparak döngüyü güvenli şekilde durdurur.
 */
static void dbuf_io_done(unsigned long cb_hndl, int err)
{
	struct xdma_io_cb  *cb    = (struct xdma_io_cb *)cb_hndl;
	struct xdma_cdev   *xcdev = (struct xdma_cdev *)cb->private;
	unsigned long       flags;
	int                 idx;

	if (!xcdev || !xcdev->dbuf_active)
		return;

	/* cb pointer'ından hangi slot olduğunu bul */
	idx = (cb == &xcdev->dbuf[1].cb) ? 1 : 0;

	if (err) {
		pr_err("dbuf[%d] DMA error %d, stopping double buffer\n", idx, err);
		spin_lock_irqsave(&xcdev->dbuf_lock, flags);
		xcdev->dbuf_active = false;
		spin_unlock_irqrestore(&xcdev->dbuf_lock, flags);
		wake_up_interruptible(&xcdev->dbuf_wq);
		return;
	}

	spin_lock_irqsave(&xcdev->dbuf_lock, flags);
	xcdev->dbuf[idx].filled = true;
	spin_unlock_irqrestore(&xcdev->dbuf_lock, flags);
...
/*
 * Bir slot için sg_table oluştur (dma_alloc_coherent adresi, 1 entry).
 * dma_mapped=true ile submit edileceği için dma_map_sg'ye gerek yok.
 */
static int dbuf_slot_init_sgt(struct xdma_dbuf_slot *slot)
{
	struct scatterlist *sg;
	int rv;

	rv = sg_alloc_table(&slot->sgt, 1, GFP_KERNEL);
	if (rv)
		return rv;

	sg = slot->sgt.sgl;
	sg_set_buf(sg, slot->virt, slot->size);
	/* dma_alloc_coherent verdiği bus adresini doğrudan yaz */
	sg_dma_address(sg) = slot->bus;
	sg_dma_len(sg)     = slot->size;

	return 0;
}

/*
 * Bir slot için async C2H DMA gönder.
 * dma_mapped=true → libxdma sg_dma_address/sg_dma_len'i kullanır, map etmez.
 */
static int dbuf_submit_slot(struct xdma_cdev *xcdev, int idx)
{
	struct xdma_dbuf_slot *slot  = &xcdev->dbuf[idx];
	struct xdma_engine    *eng   = xcdev->engine;
	struct xdma_dev       *xdev  = xcdev->xdev;
	int rv;

	slot->filled        = false;
	slot->cb.buf        = NULL;
	slot->cb.len        = slot->size;
	slot->cb.ep_addr    = xcdev->dbuf_ep_addr;
	slot->cb.write      = false;
	slot->cb.private    = xcdev;
	slot->cb.io_done    = dbuf_io_done;

	rv = xdma_xfer_submit_nowait((void *)&slot->cb, xdev,
				     eng->channel, false,
				     xcdev->dbuf_ep_addr, &slot->sgt,
				     1 /* dma_mapped */,
				     c2h_timeout * 1000);

	/* -EIOCBQUEUED (=-529) = transfer kuyruğa alındı, başarı */
	if (rv == -EIOCBQUEUED)
		return 0;

	if (rv < 0)
		pr_err("dbuf[%d] submit_nowait failed: %d\n", idx, rv);

	return rv;
}

/* IOCTL: IOCTL_XDMA_DBUF_START */
static int ioctl_do_dbuf_start(struct xdma_cdev *xcdev, unsigned long arg)
{
	struct xdma_dbuf_ioctl  io;
	struct pci_dev         *pdev = xcdev->xdev->pdev;
	int i, rv;

	if (xcdev->dbuf_active) {
		pr_err("dbuf already active\n");
		return -EBUSY;
	}

	if (copy_from_user(&io, (struct xdma_dbuf_ioctl __user *)arg, sizeof(io)))
		return -EFAULT;

	if (io.version != XDMA_DBUF_V1) {
		pr_err("dbuf unsupported version %u\n", io.version);
		return -EINVAL;
	}
	if (!io.buf_size || (io.buf_size & (PAGE_SIZE - 1))) {
		pr_err("dbuf buf_size %u must be nonzero and page-aligned\n",
		       io.buf_size);
		return -EINVAL;
	}

	xcdev->dbuf_size    = io.buf_size;
	xcdev->dbuf_ep_addr = io.ep_addr;
	xcdev->dbuf_read_idx = 0;
	init_waitqueue_head(&xcdev->dbuf_wq);
	spin_lock_init(&xcdev->dbuf_lock);

	/* Her slot için DMA-coherent bellek ayır ve sg_table kur.
	 * AXI-ST modunda FPGA descriptor sınırında 1 sample (32 byte) fazla
	 * gönderebilir. buf_size + 64 byte ayırarak taşmanın bitişik kernel
	 * belleğine yazmasını önlüyoruz. Descriptor ve copy_to_user hâlâ
	 * tam buf_size byte kullanır — fazla byte sessizce padding'e düşer. */
	for (i = 0; i < XDMA_DBUF_NUM; i++) {
		xcdev->dbuf[i].size = io.buf_size;
		xcdev->dbuf[i].virt = dma_alloc_coherent(&pdev->dev,
							  io.buf_size + 64,
							  &xcdev->dbuf[i].bus,
							  GFP_KERNEL);
		if (!xcdev->dbuf[i].virt) {
			pr_err("dbuf[%d] dma_alloc_coherent failed\n", i);
			rv = -ENOMEM;
			goto err_free;
		}

		rv = dbuf_slot_init_sgt(&xcdev->dbuf[i]);
		if (rv) {
			pr_err("dbuf[%d] sg_alloc_table failed\n", i);
			dma_free_coherent(&pdev->dev, io.buf_size,
					  xcdev->dbuf[i].virt,
					  xcdev->dbuf[i].bus);
			xcdev->dbuf[i].virt = NULL;
			goto err_free;
		}
	}

	xcdev->dbuf_active = true;

	/* İki DMA'yı sırayla engine kuyruğuna ekle */
	for (i = 0; i < XDMA_DBUF_NUM; i++) {
		rv = dbuf_submit_slot(xcdev, i);
		if (rv < 0) {
			xcdev->dbuf_active = false;
			goto err_free_sgt;
		}
	...
	pr_info("dbuf started: 2 × %u byte, ep=0x%llx\n",
		io.buf_size, io.ep_addr);
	return 0;

err_free_sgt:
	for (i = 0; i < XDMA_DBUF_NUM; i++)
		sg_free_table(&xcdev->dbuf[i].sgt);
err_free:
	for (i = 0; i < XDMA_DBUF_NUM; i++) {
		if (xcdev->dbuf[i].virt) {
			dma_free_coherent(&pdev->dev, xcdev->dbuf[i].size + 64,
					  xcdev->dbuf[i].virt,
					  xcdev->dbuf[i].bus);
			xcdev->dbuf[i].virt = NULL;
		}
...
/* IOCTL: IOCTL_XDMA_DBUF_STOP */
static int ioctl_do_dbuf_stop(struct xdma_cdev *xcdev)
{
	struct pci_dev *pdev = xcdev->xdev->pdev;
	unsigned long   flags;
	int i;
		...
		size_t count, loff_t *pos, bool write)
{
	...
	/* AXI-ST modunda FPGA descriptor boyutundan fazla byte gönderebilir.
	 * Kullanıcının istediğinden fazlasını döndürmemek için kırp. */
...
{
	struct xdma_cdev      *xcdev = (struct xdma_cdev *)file->private_data;
	struct xdma_dbuf_slot *slot;
	int                    idx;
	int                    rv;

	/* Double buffer aktif değilse eski tek-buffer yolu */
	if (!xcdev || !xcdev->dbuf_active)
		return char_sgdma_read_write(file, buf, count, pos, 0);

	/* count, slot boyutundan büyükse hata — kullanıcı dbuf_size kullanmalı */
	if (count > xcdev->dbuf_size) {
		pr_err("dbuf read: count %zu > buf_size %zu\n",
		       count, xcdev->dbuf_size);
		return -EINVAL;
	}

	idx  = xcdev->dbuf_read_idx;
	slot = &xcdev->dbuf[idx];

	/* Bu slot DMA tamamlanana kadar bekle */
	rv = wait_event_interruptible(xcdev->dbuf_wq,
				      slot->filled || !xcdev->dbuf_active);
	if (rv)
		return -ERESTARTSYS;
	...
	/* Kernel buffer → kullanıcı alanı kopyala */
	if (copy_to_user(buf, slot->virt, count))
		return -EFAULT;

	/* Bu slot'u hemen yeniden DMA'ya ver (pipeline boşalmasın) */
	rv = dbuf_submit_slot(xcdev, idx);
	if (rv < 0)
		return rv;

	/* Sıradaki slot'a geç (ping-pong) */
		...
		unsigned long arg)
{
	...
	case IOCTL_XDMA_APERTURE_W:
		...
		break;
	case IOCTL_XDMA_DBUF_START:
		if (engine->dir != DMA_FROM_DEVICE) {
			pr_err("DBUF_START: yalnızca C2H kanallarında geçerli\n");
			rv = -EINVAL;
		} else {
			rv = ioctl_do_dbuf_start(xcdev, arg);
		}
		break;
	case IOCTL_XDMA_DBUF_STOP:
		rv = ioctl_do_dbuf_stop(xcdev);
		break;
	default:
...
static int char_sgdma_close(struct inode *inode, struct file *file)
{
	...
	/* Dosya kapanırken double buffer aktifse belleği temizle */
	if (xcdev->dbuf_active)
		ioctl_do_dbuf_stop(xcdev);
