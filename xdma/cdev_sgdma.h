...
/* Double buffer (ping-pong) C2H streaming — LFSR sürekliliği için */
#define XDMA_DBUF_V1            1

struct xdma_dbuf_ioctl {
	uint32_t version;	/* XDMA_DBUF_V1 olmalı */
	uint32_t buf_size;	/* her bir DMA buffer boyutu (byte) */
	uint64_t ep_addr;	/* FPGA endpoint adresi (AXI-ST için 0) */
};
