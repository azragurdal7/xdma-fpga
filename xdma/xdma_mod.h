...
/*
 * Double buffer (ping-pong) context — FPGA→PC C2H LFSR sürekliliği için.
 * FPGA LFSR üretirken DMA pipeline hiç boşalmamalı; iki buffer sırayla
 * doldurulur, CPU birini okurken diğeri DMA kuyruğunda bekler.
 */
#define XDMA_DBUF_NUM   2

struct xdma_dbuf_slot {
	void		*virt;		/* dma_alloc_coherent kernel sanal adresi */
	dma_addr_t	 bus;		/* PCIe bus (DMA) adresi */
	size_t		 size;		/* bu slot'un byte cinsinden boyutu */
	struct sg_table	 sgt;		/* 1 entry'li SG tablosu, xdma_xfer'e gider */
	struct xdma_io_cb cb;		/* async tamamlanma bilgisi */
	volatile bool	 filled;	/* DMA tamamlandı → CPU okumaya hazır */
};
...
struct xdma_cdev {
	...
	/* Double buffer alanları (yalnızca C2H kanalı, dbuf_active=true iken geçerli) */
	struct xdma_dbuf_slot	dbuf[XDMA_DBUF_NUM];
	size_t			dbuf_size;
	u64			dbuf_ep_addr;
	int			dbuf_read_idx;	/* CPU'nun sıradaki okuyacağı slot */
	wait_queue_head_t	dbuf_wq;	/* filled geçişlerinde uyandırılır */
